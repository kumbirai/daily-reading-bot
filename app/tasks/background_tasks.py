...
from app.services.daily_reading_service import (DR_KEY, FORMAT, JFT_KEY, ReadingScraper, ReadingStorage, SPAD_KEY)

logger = logging.getLogger(__name__)

# Create a blueprint for background tasks
background_tasks = Blueprint('background_tasks', __name__)
api = Api(background_tasks, title='Background Tasks API', version='1.0', description='API for managing background tasks')

# Define models for Swagger documentation
error_model = api.model('Error', {'status': fields.String(description='Status of the response', example='error'), 'message': fields.String(description='Error message', example='Task failed'),
    'code': fields.Integer(description='HTTP status code', example=500),
    'details': fields.Raw(description='Additional error details', example={'error_type': 'TaskError', 'timestamp': '2025-04-24T17:21:45.227000', 'error_message': 'Failed to execute background task',
...
def setup_background_tasks(app):
    ...
    scheduler.add_job(func=scrape_daily_readings, trigger=IntervalTrigger(hours=3), id='periodic_scraping', name='Periodic scrape every 3 hours', replace_existing=True)
...
def scrape_daily_readings():
    ...
    try:
        ...
        logger.error(f"Critical error during daily readings scrape: {str(e)}", exc_info=True)  # Here you could add alerting or notification logic


@background_tasks.route('/scrape', methods=['POST'])
@api.doc('trigger_scrape', responses={200: 'Scraping Completed Successfully', 500: 'Internal Server Error'})
@api.marshal_with(success_model)
def trigger_scrape():
    """Manually trigger scraping of daily readings."""
    try:
        scrape_daily_readings()
        return api.marshal({'status': 'success', 'message': f'Scraping completed successfully {datetime.datetime.now(datetime.timezone.utc).astimezone().isoformat()}'}, success_model), 200
    except Exception as e:
        logger.error(f"Error triggering manual scrape: {str(e)}", exc_info=True)
        return api.marshal({'status': 'error', 'message': str(e), 'code': 500}, error_model), 500
